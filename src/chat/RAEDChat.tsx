...
export const RAEDChat = ({ embedded = false }: RAEDChatProps) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Check if the response looks like a business plan or model to show in split view
      if (text.includes('خطة عمل') || text.includes('نموذج عمل') || text.length > 500) {
  ...
  if (!isOpen && !embedded) {
    return (
      <button
        ...
        className="fixed bottom-24 right-6 w-14 h-14 bg-[#FFD700] rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-transform z-[60] group md:bottom-6"
      >
  ...
  const containerClasses = embedded 
    ...
    : `fixed right-6 z-50 bg-[#141517] border border-white/10 shadow-2xl rounded-2xl overflow-hidden flex flex-col transition-all duration-300 ${
        isMinimized 
          ? 'bottom-6 w-80 h-16' 
          : activeOutput 
            ? 'bottom-6 w-[90vw] lg:w-[1000px] h-[600px] max-h-[85vh]' 
      ...
      >
        ...
        <div className={`bg-gradient-to-r from-[#FFD700]/20 to-transparent p-4 flex justify-between items-center border-b border-white/5 ${!embedded && 'cursor-pointer'}`} onClick={() => !embedded && setIsMinimized(!isMinimized)}>
          ...
          {!embedded && (
            <div className="flex items-center gap-2 text-gray-400">
              {activeOutput && (
                <button onClick={(e) => { e.stopPropagation(); setActiveOutput(null); }} className="hover:text-white transition-colors text-xs flex items-center gap-1 bg-white/5 px-2 py-1 rounded-lg">
                  <X size={12} /> إغلاق العرض الجانبي
                </button>
              )}
        ...
        {(!isMinimized || embedded) && (
          <div className="flex-1 flex overflow-hidden">
            {/* Chat Messages */}
            <div className={`flex flex-col h-full transition-all duration-300 ${activeOutput ? 'w-full lg:w-1/3 border-l border-white/5' : 'w-full'}`}>
                ...
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`max-w-[85%] p-3 rounded-2xl text-sm leading-relaxed ${
                      msg.role === 'user' 
                        ? 'bg-[#FFD700] text-black rounded-tr-sm shadow-lg shadow-[#FFD700]/10' 
              ...
              {/* Input Area */}
              <div className="p-4 border-t border-white/5 bg-[#0B0C0E] sticky bottom-0">
                <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl p-1 pr-4 focus-within:border-[#FFD700]/50 transition-colors">
                  <input
                    type="text"
                    value={input}
                  ...
                  <button
                    ...
                    className="w-10 h-10 bg-[#FFD700] rounded-lg flex items-center justify-center text-black disabled:opacity-50 hover:bg-[#FFC000] transition-all shadow-lg shadow-[#FFD700]/20"
                  >
            ...
            {/* Side Output View (Desktop only or full overlay on mobile) */}
            {activeOutput && (
              <div className="hidden lg:flex flex-1 flex-col h-full bg-black/20 overflow-hidden">
                <div className="p-4 border-b border-white/5 flex justify-between items-center bg-white/5">
                  <div className="flex items-center gap-2 text-[#FFD700]">
                    <Sparkles size={16} />
                    <span className="font-bold text-sm">تحليل RAED الذكي</span>
                  </div>
                  <div className="flex gap-2">
                    <button className="text-xs bg-white/5 hover:bg-white/10 px-3 py-1 rounded-lg transition-colors">تصدير PDF</button>
                    <button className="text-xs bg-[#FFD700] text-black font-bold px-3 py-1 rounded-lg transition-colors">حفظ في المشروع</button>
                  </div>
                </div>
                <div className="flex-1 overflow-y-auto p-8 prose prose-invert prose-sm max-w-none custom-scrollbar">
                  <div className="whitespace-pre-wrap text-gray-300 leading-relaxed font-arabic">
                    {activeOutput}
                  </div>
                </div>
              </div>
