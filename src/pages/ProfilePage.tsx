...
import { PassionSurvey } from '../components/PassionSurvey';
...
export const ProfilePage = ({ isMe = false }: { isMe?: boolean }) => {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        // Check passion profile
        const { data: passionData } = await supabase
          .from('passion_profiles')
          .select('user_id')
          .eq('user_id', profileId)
          .single();
        setHasPassionProfile(!!passionData);
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-20 pt-24 px-4">
      {/* Passion Survey Banner */}
      {isOwnProfile && !hasPassionProfile && (
        <div className="bg-[#FFD700]/10 border border-[#FFD700]/20 rounded-2xl p-6 mb-6 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-bold text-[#FFD700] mb-1">أكمل ملف الشغف الخاص بك</h3>
            <p className="text-sm text-gray-400">ساعد "رائد" في مطابقتك مع الشركاء المثاليين بناءً على قيمك.</p>
          </div>
          <button 
            onClick={() => setShowPassionSurvey(true)}
      ...
      {showPassionSurvey && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <PassionSurvey onComplete={() => { setShowPassionSurvey(false); setHasPassionProfile(true); }} />
        </div>
      )}
      ...
      <div className="relative">
        ...
        <div className="h-48 md:h-64 rounded-3xl overflow-hidden bg-gray-800 relative group">
          {profile.cover_url ? (
            ...
            <div className="w-full h-full bg-gradient-to-r from-gray-800 via-gray-700 to-gray-900" />
          )}
          {isOwnProfile && (
          ...
          <div className="flex justify-between items-end -mt-16 mb-4">
            <div className="relative">
              <div className="w-32 h-32 rounded-full border-4 border-[#141517] overflow-hidden bg-gray-700">
                <img 
                  src={profile.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(profile.name || 'User')}&background=141517&color=FFD700`} 
                  alt={profile.name} 
                  className="w-full h-full object-cover" 
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                ...
                {profile.linkedin && (
                  <div className="flex items-center gap-2 text-gray-300">
                    ...
                    {profile.linkedin_verified && <CheckCircle2 size={14} className="text-blue-400" />}
                  </div>
                )}
                {isOwnProfile && !profile.linkedin_verified && (
                  <button className="flex items-center gap-2 text-xs bg-blue-600/10 text-blue-400 border border-blue-600/20 px-3 py-1 rounded-lg hover:bg-blue-600/20 transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/></svg>
                    {t('verifyLinkedIn')}
                  </button>
                )}
                {isOwnProfile && !profile.github_verified && (
                  <button className="flex items-center gap-2 text-xs bg-white/5 text-gray-300 border border-white/10 px-3 py-1 rounded-lg hover:bg-white/10 transition-colors">
