...
export const HomePage = () => {
  ...
  const statCards = [
    { label: 'إجمالي المستخدمين', value: stats.profileViews, icon: Users, color: 'text-blue-500', bg: 'bg-blue-500/10' },
    { label: 'الأفكار المطروحة', value: stats.interactions, icon: Activity, color: 'text-green-500', bg: 'bg-green-500/10' },
    { label: 'المستثمرون', value: stats.partnerships, icon: Target, color: 'text-purple-500', bg: 'bg-purple-500/10' },
  ];

  return (
    <div className="min-h-screen bg-[#0B0C0E] pt-24 pb-24 px-4 relative overflow-hidden w-full max-w-[100vw]">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-[#141517] p-8 rounded-3xl border border-white/5 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h1 className="text-3xl font-bold text-white mb-2 flex flex-wrap items-center gap-2">
              مرحباً بعودتك، <span className="text-[#FFD700]">{user?.user_metadata?.full_name || user?.email?.split('@')[0] || 'المستخدم'}</span>
