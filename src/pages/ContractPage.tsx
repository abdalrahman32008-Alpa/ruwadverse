...
import { FileText, Download, CheckCircle, PenTool, Calendar, Shield, Users, Loader2 } from 'lucide-react';
...
export const ContractPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchContract = async () => {
      try {
        // Fetch the latest contract for the user (mocking the query logic as we don't have a clear relation in schema for 'parties' search without complex query)
        // Ideally we would query contracts where user is in parties array.
        // For now, let's just fetch the latest contract created by ANYONE (as a demo) OR just return null if we want to be strict.
        // But to show something, let's try to find a contract where user is involved if possible.
        // Since `parties` is jsonb, we can use @> operator.
        
        // const { data, error } = await supabase
        //   .from('contracts')
        //   .select('*')
        //   .contains('parties', [{ id: user.id }]) // Assuming parties stores IDs
        //   .limit(1)
        //   .single();
        
        // Since I don't know the exact structure of JSONB data in existing rows (if any), 
        // and to comply with "remove mock data", I will try to fetch. If empty, show empty state.
        ...
        if (data) {
            // Parse terms if it's a string, or split by newline
            setContract({
                ...data,
  ...
  const handleSign = async () => {
    ...
    try {
        ...
        setContract(prev => prev ? { ...prev, signed: true } : null);
    } catch (error) {
  ...
  const termsList = contract.terms.split('\n').filter(t => t.trim() !== '');
          ...
          <div ref={contractRef} className="bg-[#141517] p-4 rounded-xl">
            ...
            <div className="flex justify-between items-start mb-8 border-b border-white/5 pb-6">
              <div>
                ...
                <span className="text-sm text-gray-400">رقم العقد: {contract.id.slice(0, 8).toUpperCase()}</span>
              </div>
              <div className="text-left">
                <span className="block text-sm text-gray-400 mb-1">تاريخ الإنشاء</span>
                <span className="font-mono text-white">{new Date(contract.created_at).toLocaleDateString('ar-SA')}</span>
              </div>
            </div>
