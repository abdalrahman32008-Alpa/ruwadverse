    ...
    const fetchData = async () => {
      try {
        ...
        const mappedPosts: Post[] = ideasData?.map(idea => ({
            id: idea.id,
            user: {
                name: idea.owner?.full_name || t('userFallback'),
                avatar: idea.owner?.avatar_url || '',
                role: idea.owner?.user_type || t('memberRole')
            },
            content: idea.description,
            time: new Date(idea.created_at).toLocaleDateString(t('locale')),
        ...
        setSuggestedPeople(usersData?.map(u => ({
            id: u.id,
            name: u.full_name || t('userFallback'),
            avatar_url: u.avatar_url || '',
            role: u.user_type || t('memberRole')
        })) || []);
            ...
            <div className="space-y-4">
              {loading ? (
                  ...
                  suggestedPeople.map((person) => (
                    ...
                    <button className="text-[#FFD700] text-xs font-bold hover:underline">{t('followBtn')}</button>
                    </div>
                ))
              ) : (
