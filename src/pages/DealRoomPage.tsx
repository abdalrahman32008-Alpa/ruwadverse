...
import { Shield, FileText, CheckCircle2, Lock, FileSignature, Upload, Download, Users, Loader2, ArrowRight, MessageSquare, Send, Coins, Network } from 'lucide-react';
import { supabase } from '../lib/supabase';
import { useAuth } from '../contexts/AuthContext';
import { toast } from 'react-hot-toast';
import { SmartEquity } from '../components/futuristic/SmartEquity';
import { QuantumCapTable } from '../components/futuristic/QuantumCapTable';
...
export const DealRoomPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<'documents' | 'terms' | 'signatures' | 'smart-equity' | 'quantum-cap'>('documents');
    ...
    <div className="min-h-screen bg-[#0B0C0E] pt-24 pb-20 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex gap-4 border-b border-white/10 mb-8 overflow-x-auto hide-scrollbar">
          ...
          <button 
            onClick={() => setActiveTab('smart-equity')}
            className={`pb-4 px-4 font-bold whitespace-nowrap transition-colors border-b-2 ${activeTab === 'smart-equity' ? 'border-purple-400 text-purple-400' : 'border-transparent text-gray-400 hover:text-white'}`}
          >
            <div className="flex items-center gap-2">
              <Coins size={18} /> العقود الذكية
            </div>
          </button>
          <button 
            onClick={() => setActiveTab('quantum-cap')}
            className={`pb-4 px-4 font-bold whitespace-nowrap transition-colors border-b-2 ${activeTab === 'quantum-cap' ? 'border-cyan-400 text-cyan-400' : 'border-transparent text-gray-400 hover:text-white'}`}
          >
            <div className="flex items-center gap-2">
              <Network size={18} /> جدول الحصص الكمّي
            </div>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {activeTab === 'smart-equity' && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <SmartEquity />
              </motion.div>
            )}

            {activeTab === 'quantum-cap' && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <QuantumCapTable />
              </motion.div>
            )}
