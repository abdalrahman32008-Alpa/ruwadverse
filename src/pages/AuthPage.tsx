...
type AuthMode = 'login' | 'register' | 'forgot_password' | 'check_email';
...
export const AuthPage = () => {
  ...
  const handleResendLink = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success('تم إعادة إرسال رابط التأكيد إلى بريدك الإلكتروني');
    } catch (err: any) {
      ...
      <div className="w-full max-w-md p-8 rounded-3xl bg-[#141517]/80 backdrop-blur-md border border-white/10 shadow-2xl relative z-10">
        ...
        {mode === 'check_email' ? (
          <div>
            ...
            <p className="text-gray-400 text-center mb-8">أرسلنا رابط تأكيد إلى {email}. يرجى الضغط على الرابط لتفعيل حسابك.</p>
            
            {error && <div className="p-4 mb-6 bg-red-500/10 border border-red-500/20 text-red-500 rounded-xl text-sm">{error}</div>}
            
            <div className="space-y-6">
              <button type="button" onClick={handleResendLink} disabled={loading} className="w-full bg-[#FFD700] text-black font-bold py-3 rounded-xl hover:bg-[#FFC000] transition-colors disabled:opacity-50">
                {loading ? 'جاري الإرسال...' : 'إعادة إرسال رابط التأكيد'}
              </button>
              <button type="button" onClick={() => setMode('login')} className="w-full text-gray-400 hover:text-white text-sm transition-colors">
