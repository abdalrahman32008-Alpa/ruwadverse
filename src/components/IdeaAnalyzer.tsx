...
import { Lightbulb, Target, TrendingUp, Shield, Users, AlertTriangle, CheckCircle, Search, RefreshCw, BarChart2, Lock } from 'lucide-react';
    ...
    try {
      ...
      try {
          const embeddingResult = await ai.models.embedContent({
            ...
            contents: { parts: [{ text: `${ideaData.title} ${ideaData.problem} ${ideaData.market_size}` }] },
          });
          ...
          const { data, error: searchError } = await supabase.rpc('match_ideas', {
          ...
          // Fail silently for search if embedding fails
      }
              ...
              <div className="relative overflow-hidden rounded-xl">
                {subscriptionTier === 'free' && (
                  <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center z-10">
                    <Lock className="text-[#FFD700] mb-2" size={24} />
                    <p className="text-white font-bold text-sm mb-2">توصيات RAED المتقدمة</p>
                    <Link to="/pricing" className="text-[#FFD700] text-xs hover:underline border border-[#FFD700] px-3 py-1 rounded-full">
                ...
                <ul className={`text-sm text-gray-300 space-y-2 bg-white/5 p-4 rounded-xl border border-white/10 ${subscriptionTier === 'free' ? 'blur-sm' : ''}`}>
                ...
                <div className="relative overflow-hidden rounded-xl">
                  {subscriptionTier === 'free' && (
                    <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center z-10">
                      ...
                      <p className="text-white font-bold text-sm mb-2">كشف الأفكار المشابهة</p>
                      <Link to="/pricing" className="text-purple-400 text-xs hover:underline border border-purple-400 px-3 py-1 rounded-full">
                        ترقية للعضوية المميزة
                      </Link>
                    </div>
                  )}
                  ...
                  <div className={`space-y-2 ${subscriptionTier === 'free' ? 'blur-sm' : ''}`}>
