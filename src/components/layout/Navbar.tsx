      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-center space-x-8 space-x-reverse rtl:space-x-reverse ltr:space-x-reverse">
              <button onClick={() => handleNavigate(user ? '/home' : '/')} className="text-sm text-gray-400 hover:text-white transition-colors">{t('home')}</button>
              <button onClick={() => handleNavigate('/feed')} className="text-sm text-gray-400 hover:text-white transition-colors">المجتمع</button>
              <button onClick={() => handleNavigate('/marketplace')} className="text-sm text-gray-400 hover:text-white transition-colors">سوق الأفكار</button>
              <button onClick={() => handleNavigate('/support')} className="text-sm text-gray-400 hover:text-white transition-colors">{t('supportTitle')}</button>
              ...
              {onSearch && (
                ...
                >
                  <Search size={14} />
                  <span>بحث...</span>
                  <span className="text-[10px] bg-white/20 px-1.5 py-0.5 rounded border border-white/10 ml-2 opacity-70">Ctrl+K</span>
                </button>
              )}
              
              {user && (
                <>
                  <button onClick={() => handleNavigate('/raed')} className="text-sm text-gray-400 hover:text-white transition-colors">رائد (RAED)</button>
                  <button onClick={() => handleNavigate('/messages')} className="text-sm text-gray-400 hover:text-white transition-colors">الرسائل</button>
                  <button onClick={() => handleNavigate('/dashboard')} className="text-sm text-gray-400 hover:text-white transition-colors">لوحة التحكم</button>
                  <button onClick={() => handleNavigate('/settings')} className="text-sm text-gray-400 hover:text-white transition-colors">الإعدادات</button>
                </>
              )}
                ...
                <div className="absolute top-full left-0 mt-2 w-48 bg-[#141517] border border-white/10 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                  ...
                  {user && (
                    <>
                      ...
                      <button onClick={() => handleNavigate('/deal-room')} className="block w-full text-start px-4 py-2 text-sm text-gray-300 hover:bg-white/5 hover:text-[#FFD700]">غرفة الصفقات</button>
                    </>
                  )}
                </div>
              </div>
              ...
              {!user && (
                <>
                  <a href="#features" className="text-sm text-gray-400 hover:text-white transition-colors">{t('features')}</a>
                  <a href="#features" className="text-sm bg-white/10 px-4 py-2 rounded-full hover:bg-white/20 transition-colors">Learn More</a>
                </>
              )}
              
              {user && <div className=""><Notifications /></div>}

              {onFeedback && (
                <button onClick={onFeedback} className="text-gray-400 hover:text-white transition-colors" title={t('feedback')} aria-label={t('feedback')}>
                  <MessageSquare size={18} aria-hidden="true" />
                </button>
              )}
              
              <button onClick={toggleLanguage} className="text-gray-400 hover:text-white transition-colors flex items-center gap-1" aria-label={language === 'ar' ? 'Switch to English' : 'Switch to Arabic'}>
              ...
              {user ? (
                <button onClick={signOut} className="text-sm font-medium bg-white/10 hover:bg-white/20 text-white px-4 py-1.5 rounded-full transition-all border border-white/5">
                  تسجيل خروج
                </button>
              ) : (
                <button onClick={() => handleNavigate('/auth')} className="text-sm font-medium bg-white/10 hover:bg-white/20 text-white px-4 py-1.5 rounded-full transition-all border border-white/5">
                  {t('startNow')}
                </button>
              )}
            </div>
