...
export const Marketplace = () => {
  ...
  const loadMore = () => {
    ...
    setTimeout(() => {
      const newIdeas = mockIdeas.map(idea => ({
        ...idea,
        id: idea.id + ideas.length, // Generate unique ID
        title: `فكرة #${idea.id + ideas.length} — ${idea.sector === 'Education' ? 'قطاع التعليم' : idea.sector === 'FinTech' ? 'التقنية المالية' : idea.sector === 'HealthTech' ? 'الصحة' : idea.sector === 'E-commerce' ? 'التجارة' : 'العقارات'}`
      }));
      setIdeas(prev => [...prev, ...newIdeas]);
      setLoadingMore(false);
      
      // Stop after loading 3 pages (for demo purposes)
      if (ideas.length >= 15) {
        setHasMore(false);
      }
    }, 1000);
