...
const Navbar = ({ onFeedback, onSearch, onToggleSidebar }: { onFeedback: () => void, onSearch: () => void, onToggleSidebar?: () => void }) => {
          ...
          >
            <div className="px-6 py-8 flex flex-col gap-6">
              <div className="flex flex-col gap-4">
                ...
                <button onClick={() => handleNavigate(user ? '/home' : '/')} className="text-start text-xl font-bold text-white hover:text-[#FFD700] transition-colors">{t('home')}</button>
                <button onClick={() => handleNavigate('/feed')} className="text-start text-xl font-bold text-white hover:text-[#FFD700] transition-colors">المجتمع</button>
                <button onClick={() => handleNavigate('/marketplace')} className="text-start text-xl font-bold text-white hover:text-[#FFD700] transition-colors">سوق الأفكار</button>
                <button onClick={() => handleNavigate('/support')} className="text-start text-xl font-bold text-white hover:text-[#FFD700] transition-colors">{t('supportTitle')}</button>
              </div>

              {user && (
                <div className="flex flex-col gap-4 pt-6 border-t border-white/10">
                  <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider">لوحة التحكم</h4>
                  <button onClick={() => handleNavigate('/messages')} className="text-start text-xl font-bold text-white hover:text-[#FFD700] transition-colors">الرسائل</button>
                  <button onClick={() => handleNavigate('/dashboard')} className="text-start text-xl font-bold text-white hover:text-[#FFD700] transition-colors">لوحة التحكم</button>
                  <button onClick={() => handleNavigate('/referral')} className="text-start text-xl font-bold text-white hover:text-[#FFD700] transition-colors">برنامج الإحالة</button>
                  <button onClick={() => handleNavigate('/contract')} className="text-start text-xl font-bold text-white hover:text-[#FFD700] transition-colors">العقود</button>
                </div>
              )}

              <div className="flex flex-col gap-4 pt-6 border-t border-white/10">
                <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider">الملفات الشخصية</h4>
                <button onClick={() => handleNavigate('/profile-skill')} className="text-start text-lg text-gray-300 hover:text-[#FFD700] transition-colors">ملف المهارة</button>
                <button onClick={() => handleNavigate('/profile-founder')} className="text-start text-lg text-gray-300 hover:text-[#FFD700] transition-colors">ملف المؤسس</button>
                <button onClick={() => handleNavigate('/profile-investor')} className="text-start text-lg text-gray-300 hover:text-[#FFD700] transition-colors">ملف المستثمر</button>
              </div>

              <div className="flex flex-col gap-4 pt-6 border-t border-white/10">
                <button onClick={() => { onFeedback(); setIsOpen(false); }} className="text-start text-lg text-gray-300 hover:text-[#FFD700] transition-colors flex items-center gap-2">
                  <MessageSquare size={18} /> {t('feedback')}
                </button>
                {user ? (
                  <button onClick={() => { signOut(); setIsOpen(false); }} className="text-start text-lg text-red-400 hover:text-red-300 transition-colors mt-4">تسجيل خروج</button>
                ) : (
  ...
  return (
    <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
      ...
      <motion.div 
        ...
        className="absolute top-40 left-10 w-32 h-32 bg-purple-500/10 rounded-full blur-2xl"
      />
      <motion.div 
        animate={{ y: [0, 30, 0], x: [0, 20, 0] }} 
        transition={{ duration: 8, repeat: Infinity, ease: "easeInOut", delay: 1 }}
        className="absolute bottom-40 right-10 w-48 h-48 bg-[#FFD700]/10 rounded-full blur-3xl"
      />
      <motion.div 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        ...
        >
          <motion.div 
            ...
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-xs text-[#FFD700] mb-8 animate-float cursor-default"
          >
            <Sparkles size={12} />
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4 items-center mb-16">
            <motion.button 
              ...
              className="btn-linear-primary px-8 py-4 rounded-full text-lg min-w-[200px]"
            >
            ...
            <motion.button 
              ...
              className="btn-linear-secondary px-8 py-4 rounded-full text-lg min-w-[200px] flex items-center justify-center gap-2 group"
            >
          ...
          <motion.div 
            ...
            className="max-w-xl mx-auto mb-16 relative z-20"
          >
            <div className="bg-[#141517]/80 backdrop-blur-md border border-white/10 rounded-full p-2 flex items-center shadow-2xl hover:border-white/20 transition-colors">
              <div className="pl-4 pr-2 text-gray-400">
                <Lightbulb size={20} className="animate-pulse" />
              </div>
              <input 
                type="text" 
                aria-label="ما هي الفكرة التي تريد بناءها؟"
                placeholder="ما هي الفكرة التي تريد بناءها؟ (مثال: تطبيق لتوصيل الطعام الصحي)" 
                className="bg-transparent border-none outline-none text-white w-full placeholder-gray-500 text-sm"
              />
              <button 
                onClick={() => navigate('/auth')}
                className="bg-[#FFD700] text-black font-bold px-6 py-2 rounded-full text-sm hover:bg-[#FFC000] transition-colors whitespace-nowrap"
              >
                {t('startNow')}
              </button>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl mx-auto mb-20 border-t border-b border-white/5 py-8">
            {[
              { value: t('usersCount'), label: 'Active Users' },
              { value: t('projectsCount'), label: 'Startups' },
          ...
          >
            <div className="absolute inset-0 bg-[#FFD700] blur-[100px] opacity-10 rounded-full animate-pulse" style={{ animationDuration: '4s' }} />
            <div className="relative rounded-xl border border-white/10 bg-[#141517]/50 backdrop-blur-xl overflow-hidden shadow-2xl group">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-white/5 bg-[#141517]">
                ...
                <div className="mx-auto text-xs text-gray-500 font-mono group-hover:text-[#FFD700] transition-colors">ruwadverse_dashboard.exe</div>
              </div>
              <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-6 opacity-80 group-hover:opacity-100 transition-opacity duration-500">
                {/* Fake UI Elements for Preview */}
                <div className="col-span-1 space-y-4">
                  <motion.div whileHover={{ scale: 1.02 }} className="p-4 rounded-lg bg-white/5 border border-white/5 flex flex-col gap-3 transition-transform">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#FFD700] to-orange-500 animate-spin" style={{ animationDuration: '10s' }} />
                      <div>
                  ...
                  <motion.div whileHover={{ scale: 1.02 }} className="p-4 rounded-lg bg-white/5 border border-white/5 transition-transform">
                    ...
                    <div className="space-y-3">
                      {[1, 2, 3].map(i => (
                        <div key={i} className="flex items-center justify-between">
                          ...
                          <div className="h-2 w-8 bg-[#FFD700]/50 rounded" />
                        </div>
                      ))}
                    </div>
                  </motion.div>
                ...
                <div className="col-span-1 md:col-span-2 space-y-4">
                  <div className="p-4 rounded-lg bg-white/5 border border-white/5 flex items-center justify-between">
                    <div className="flex gap-4">
                      <div className="h-8 w-24 bg-white/10 rounded-lg hover:bg-white/20 transition-colors" />
                      <div className="h-8 w-24 bg-white/10 rounded-lg hover:bg-white/20 transition-colors" />
                      <div className="h-8 w-24 bg-[#FFD700]/20 rounded-lg border border-[#FFD700]/30" />
                    </div>
                    <div className="h-8 w-8 rounded-full bg-white/10" />
                  </div>
                  <div className="p-6 rounded-lg bg-white/5 border border-white/5 h-64 flex flex-col justify-between relative overflow-hidden">
                    ...
                    <div className="flex items-end gap-2 h-32 mt-4 relative z-10">
                      {[40, 70, 45, 90, 65, 85, 100].map((h, i) => (
                        <motion.div 
                          ...
                          className="flex-1 bg-gradient-to-t from-[#FFD700]/20 to-[#FFD700]/80 rounded-t-sm hover:opacity-80 transition-opacity" 
                        />
                      ))}
                    </div>
                  </div>
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 w-full">
              ...
              <div className="w-full md:w-1/3 bg-[#141517] border border-white/10 rounded-xl p-4 shadow-2xl">
                ...
                <div className="flex gap-2">
                  <div 
                    onClick={() => navigate('/raed')}
                    className="h-8 bg-[#FFD700] rounded-lg w-full flex items-center justify-center text-black text-xs font-bold cursor-pointer hover:bg-[#FFC000] transition-colors"
                  >
                    {t('startNow')}
                  </div>
                </div>
