      ...
      {!loading && (
        <div className="min-h-screen flex flex-col font-sans overflow-x-hidden">
          ...
          <div className={`flex flex-1 pt-16 ${user ? 'pb-24 md:pb-0' : ''}`}>
            ...
            {user && <RAEDAgent />}
            {user && <SmartMatchEngine />}
            
            {/* Toggle Button for UIAuditPanel */}
            <button 
              onClick={() => setIsAuditOpen(!isAuditOpen)}
              className="fixed bottom-24 left-6 bg-[#141517] border border-white/10 p-3 rounded-full hover:bg-white/10 transition-colors z-[60] md:bottom-6"
