...
export const generateRaedResponse = async (
  ...
  const chat = ai.chats.create({
    model: "gemini-3.1-pro-preview", // Upgraded to Pro for complex tasks
    config: {
      systemInstruction: systemInstruction,
      thinkingConfig: { thinkingLevel: ThinkingLevel.HIGH }, // Added thinking mode
...
export const generateImage = async (prompt: string, aspectRatio: string = "1:1", size: string = "1K") => {
  ...
  const model = isProAspectRatio ? "gemini-3-pro-image-preview" : "gemini-3.1-flash-image-preview";
